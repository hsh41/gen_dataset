...
def create_record(data_dir, name, parts=1, batch_size=50, mode='train'):
    ...
    img, label, bbox = iterator.get_next()
    sess = tf.Session()
    while True:
        try:
            im, ex, box = sess.run([img, label, bbox])
        ...
        for i in range(len(ex)):
            example = tf.train.Example(
                features=tf.train.Features(feature={
                    ...
                    "bbox": tf.train.Feature(int64_list=tf.train.Int64List(value=box[i]))
                }))
...
def fetch_data(data_dir, batch_size, num_epochs=1, mode='train'):
    ...
    bbox = tf.data.Dataset.from_tensor_slices(bbox)
    ...
    dataset = tf.data.Dataset.zip((features, labels, bbox))
